...
def main():
	...
	for constraint in parsed:
		...
		neighborVals['X'][participant] = 0
		neighborVals[participant]['X'] = 0
	...
	maxHappiness = 0
	participantsPermutations = list(permutations(neighborVals.keys(), 9))

	for s1, s2, s3, s4, s5, s6, s7, s8, s9 in participantsPermutations:
		curHappiness = 0
		curHappiness += neighborVals[s1][s2]
		curHappiness += neighborVals[s1][s9]
		...
		curHappiness += neighborVals[s8][s9]
		curHappiness += neighborVals[s9][s8]
		curHappiness += neighborVals[s9][s1]
